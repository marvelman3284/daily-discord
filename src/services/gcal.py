...
def get_events() -> dict:
    ...
    try:
        ...
        # DONE: get events that are in a week or less using datetime
        for event in events:
            ...
            # HACK:
            if "Workout" not in event['summary']:
                l['normal'].append(str(start) + " " + event['summary'])
            else:
                l['workout'].append(str(start) + " " + event['summary'])
