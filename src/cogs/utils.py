...
class Utils(commands.Cog):
    ...
    async def info(self, interaction: discord.Interaction) -> None:
        ...
        embed.add_field(
            name="Running Commit", value=commit, inline=False
    ...
    @app_commands.command(
        name="ping", description="Displays latency between client and bot"
    )
    ...
    async def week(self, ctx: discord.Interaction):
        ...
        embed = discord.Embed(title="Weeks Weather")
    ...
    async def daily(self):
        def get_events(category: str) -> discord.Embed:
            ...
            if category == "normal":
                cal_embed = discord.Embed(title="Upcoming events")
            else:
                cal_embed = discord.Embed(title="Upcoming workouts")
        ...
        weather_embed = discord.Embed(title="Weeks Weather")
...
async def setup(bot: commands.Bot):
    await bot.add_cog(
        Utils(bot),
        guilds=[
            discord.Object(id=664600025573359626),
            discord.Object(id=529118747521318913),
        ],
    )
